...
def f(state:State , context: dict ) -> str:
     ...
     if state.name == "STAGE1": return "input2"
...
def test_multiple_initial_stages_raise_Transition_error():
    start  = State("START", f,initial=True,)
    stage1 = State("STAGE1",f,initial=True,)
    fsm = FiniteStateMachine("TheMachine")
...
def test_attributes_are_set():
    ...
    error  = State("ERROR",f,error=True,mail_addr='[{"A":"error_handler@mail.company"}]', default=final)
