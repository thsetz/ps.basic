...
class Basic(object):
    ...
    logging_bridge_port = (
        "not_yet_defined"  #: which port is used to transmit -bridged- log_messages
    )
    ...
    primary_herald_url = (
        "not yet defined"  #: provide an url where a herald could be reached
    )
    ...
    def __init__(
        ...
        if init_p:
            ...
            if dev_stage == None or dev_stage not in DEV_STAGES.keys():
                sys.stderr.write(
                    "DEV_STAGE '%s' Not an Allowed DEVELOPMENT STAGE " % (dev_stage)
                )
            ...
            Basic.herald_sqlite_filename = os.path.join(
                os.getcwd(), "herald.sqlite%s" % Basic.suffix
            )
            Basic.primary_herald_url = ""
            Basic.lock_filename = os.path.join(
                os.getcwd(), "%s_lock%s" % (project_name_p, Basic.suffix)
            )
            ...
            if not os.path.isdir("LOG"):
                try:
                    ...
                    sys.stderr.write(
                        "Error creating LOG DIR in %s: EXIT Now" % (os.getcwd())
                    )
                    sys.stderr.write("Error creating LOG DIR: EXIT Now")
                    sys.exit(1)
            Basic.log_file_name = os.path.join(
                "LOG", Basic.name + Basic.suffix + ".log"
            )
            ...
            Basic.logger.debug(
                "Logging initialized for pid %s" % (os.getpid()),
            ...
            if guarded_by_lockfile:
                if not os.path.isfile(Basic.lock_filename):
                    try:
                        ...
                        Basic.logger.exception(
                            "Error opening/writing lock File: version %s" % (version)
                        )
                        sys.stderr.write(
                            "Error open/write lockFile %s: EXIT Now"
                            % (Basic.lock_filename)
                        )
                    ...
                    Basic.logger.info(
                        ...
                        % (Basic.name, Basic.lock_filename),
                        extra={"package_version": version},
                    )
                else:
                    ...
                    Basic.logger.info(
                        "%s locked by process with pid %s for: %s: version %s"
                        % (Basic.lock_filename, pid, age, version)
                    )
                    sys.stdout.write(
                        "%s locked by process with pid %s for %s"
                        % (Basic.lock_filename, pid, age)
                    )

                    try:
                        os.kill(int(pid), 0)
                        Basic.logger.info(
                            "process with pid %s is still alive" % (pid),
                            extra={"package_version": version},
                        )
                    except OSError:
                        Basic.logger.error(
                            "process with pid %s is not alive: Remove it's lock file"
                            % (pid),
                            extra={"package_version": version},
                        )
                        sys.stderr.write(
            ...
            if have_config_file:
                ...
                if config_file_directory:
                    if not os.path.isdir(config_file_directory):
                        Basic.logger.fatal(
                            "The BASIC_CONFIGFILE_DIR %s does not exist"
                            % (config_file_directory),
                            extra={"package_version": version},
                        )
                        sys.stderr.write(
                            "ERROR: The BASIC_CONFIGFILE_DIR %s does not exist. EXIT NOW"
                            % (config_file_directory)
                        )
                        self.__exit__(1, 2, 3)
                        sys.exit(1)
                    else:
                        Basic.config_file_name = os.path.join(
                            config_file_directory, Basic.name + Basic.suffix + ".cfg"
                        )
                        Basic.logger.info(
                            "The config_file was forced  to %s "
                            % Basic.config_file_name,
                            extra={"package_version": version},
                        )
                else:
                    Basic.config_file_name = os.path.join(
                        os.getcwd(), Basic.name + Basic.suffix + ".cfg"
                    )
                ...
                try:
                    if os.path.isfile(Basic.config_file_name):
                        ...
                        Basic.logger.debug(
                            "No config file %s given: version %s "
                            % (Basic.config_file_name, version)
                        )
                        if force_usage_of_config_file:
                            sys.stderr.write(
                                "No config file %s given but it's usage is mandatory"
                                % (Basic.config_file_name)
                            )
                            Basic.logger.fatal(
                                "No config file %s given but it's usage is mandatory: version %s"
                                % (Basic.config_file_name, version)
                            )
                            self.__exit__(1, 2, 3)
                            sys.exit(1)
                    ...
                    e = sys.exc_info()[1]
                    Basic.logger.exception(
                        "Error reading config File: version %s" % (version)
                    )
                ...
                if force_usage_of_config_file:
                    try:
                        Basic.primary_herald_url = Basic.config_parser.get(
                            "GLOBAL", "herald_url"
                        )
                    except:
                        Basic.logger.fatal(
                            "Value of herald_url in GLOBAL section of %s not given - but needed. EXIT NOW"
                            % (Basic.config_file_name),
                            extra={"package_version": version},
                        )
                        sys.stderr.write(
                            "Value of herald_url GLOBAL section of %s not given. EXIT NOW"
                            % Basic.config_file_name
                        )
                        self.__exit__(1, 2, 3)
                        sys.exit(1)
                ...
                try:
                    PATTERN_LANGUAGE = Basic.config_parser.get(
                        "GLOBAL", "pattern_language"
                    )
                    if PATTERN_LANGUAGE not in PATTERN_LANGUAGES:
                        Basic.logger.fatal(
                            "Value of pattern_language in GLOBAL section of %s not allowed. EXIT NOW"
                            % Basic.config_file_name,
                            extra={"package_version": version},
                        )
                        sys.stderr.write(
                ...
                for section_name in Basic.config_parser.sections():
                    ...
                    for name, value in Basic.config_parser.items(section_name):
                        ...
                        if name == "LOGGING":
                            Basic.logger.setLevel(int(value))
                html_string += "</table>"
    ...
    def verbose(self):
        ...
        formater = logging.Formatter(
            "%(asctime)s - %(name)-12s %(lineno)d - %(message)s"
        )
        ...
        if exc_type == 1 and exc_value == 2 and traceback == 3:
            try:
                ...
                if os.path.isfile(self.lock_filename):
                    Basic.logger.debug(
                        "remove lock_file %s" % (self.lock_filename),
                        extra={"package_version": version},
                    )
...
def ps_shell(cmd_p, env_p=None):
    ...
    if exitcode != 0:
        success = "ERROR"
    logger.debug(
        "%s %s %s " % (success, time_needed, cmd_p), extra={"package_version": version}
    )
    if exitcode != 0:
        logger.error(
            "%d %s" % (int(exitcode), stderr), extra={"package_version": version}
        )
...
def send_a_mail(
    sent_from, l_send_to_p, subject, text, files=[], server="localhost", test_only=False
):
    ...
    try:
        ...
        try:
            ...
            logger.debug(
                "Try To send via shell mail", extra={"package_version": version}
            )
            for dst in l_send_to_p:
                cmd = ' echo "%s" | mail -s "%s" %s' % (text, subject, dst)
                logger.debug(
                    "Try to send mail via shell %s" % (cmd),
                    extra={"package_version": version},
                )
                o, e, ex, t = ps_shell(cmd)
                if int(ex) > 0:
                    logger.error(
                        "FAILURE Mail via shell", extra={"package_version": version}
                    )
                else:
                    logger.info(
                        "SUCCESS Mail via shell to %s" % (dst),
                        extra={"package_version": version},
                    )
        except:
            logger.error(
                "FAILURE send_mail via shell FROM "
                + send_from
                + " TO "
                + str(l_send_to_p)
                + " subject "
...
def local_exec(
    exec_instance_p, local_shell_cmds_p, exec_cmd_shortcut_p, d_vars_p, ret_stdout=False
):
...
class EXEC(object):
    ...
    def __init__(self, sys_p, dictionary=None):
        ...
        try:
            self.data["SSH_CMD_PREFIX"] = 'ssh -o "ConnectTimeout 20"  %s' % (
                self.data["DESTINATION_HOST"]
            )
        except:
            self.logger.exception(
                "DESTINATION_HOST not set. EXEC instance will not be available for remote commands."
            )
    ...
    def lexec(self, cmd_p, stop_on_Failure=True):
        ...
        self.logger.debug(
            "%s execute Command %s" % (Basic.name, cmd),
            extra={"package_version": version},
        )
        self.stdout, self.stderr, self.exitcode, self.timings = ps_shell(cmd)
        if self.exitcode != 0:
            self.logger.error(
                "%s stdout %s: version %s" % (Basic.name, self.stdout, version)
            )
            self.logger.error(
                "%s stderr %s: version %s" % (Basic.name, self.stderr, version)
            )
    ...
    def rexec(self, cmd_p, stop_on_Failure=True):
        ...
        self.logger.debug(
            "%s remotely execute (line %d) Command %s: version %s"
            % (Basic.name, line_number, cmd_p, version)
        )
        self.data["curr_cmd"] = cmd_p
