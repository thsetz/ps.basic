...
class FiniteStateMachine(object):
    ...
    DOT_ATTRS = {"directed": True, "strict": False, "rankdir": "LR", "ratio": "0.3"}
    ...
    def transition(self, input_value):
        ...
        if destination_state is None:
            raise TransitionError(
                "Cannot transition from state %r"
    ...
    def run(self, context):
        ...
        while True:
            ...
            return_value_of_compute_function = my_current_state.compute_function(
                my_current_state, context
            )
            Basic.logger.debug(
                my_current_state.name
                + "-compute_function returned  "
                + str(return_value_of_compute_function),
                extra={"package_version": version},
            )
            if isinstance(return_value_of_compute_function, State):
                Basic.logger.debug(
                    "%s.run new step: %s's default_handler left context as %s"
            ...
            if my_current_state.final == True:
                Basic.logger.debug(
                    "Leave run, as " + self.current_state.name + " is a final state ",
                    extra={"package_version": version},
                )
                break
            else:
...
class StateComputingFunction(object):
    ...
    def __call__(self, f):
        ...
        def wrapped_f(*args):
            ret = f(*args)
            Basic.logger.debug(
                "After f(*args), which returned " + str(ret),
                extra={"package_version": version},
            )
            if ret not in self.allowed_return_values:
                Basic.logger.error(
                    "unallowed return value %s " % (str(ret))
                    + " allowed are "
                    + str(self.allowed_return_values),
                    extra={"package_version": version},
                )
                raise ComputeFunctionTransitionError(
                    "return value %s not allowed in state %s" % (str(ret), args[0].name)
                )
            return ret
...
class State(dict):
    ...
    def __init__(
        ...
        if compute_function == None:
            self.compute_function = default_compute_function
        else:
        ...
        if final:
            machine.final_states.append(self)
        if error:
            machine.error_states.append(self)
